...
It is computed from sample data and may not be representative of an entire population
...
I would not be surprised if 3 were left-handed since that's only a 4% difference, however 40 left handed students would be surprising since it is so drastically different from the last sample
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-maggiebrooks/manhattan.csv")
```
...
What does this mean?
This gives you the upper and lower bounds of where the true mean exists with 95% confidence
...
- Sample: Mice not on treatment (smaller group)
- Sample size: 522
...
What does this mean?
This shows the possible true mean of number of mice died not on treatment with 90% confidence represented by the green rectangle
...
Consider the mice radiation results. Suppose we want to compute a 95% confidence interval for the proportion of mice that lived while on the treatment, regardless of the dose.
...
This shows the possible true mean of number of mice died regardless of treatment with 90% confidence represented by the green rectangle.  This range is between 0.53-0.61 roughly.
